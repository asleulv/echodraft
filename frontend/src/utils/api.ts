...
export const exportAPI = {
  ...
  createFromDocument: async (documentIdentifier: string | number, options: {
    expiration_type?: string;
    pin_protected?: boolean;
  } = {}) => {
    const response = await api.post('/pdf-exports/create-from-document', {
      [typeof documentIdentifier === 'number' ? 'document_id' : 'document_slug']: documentIdentifier,
      ...options
    });
...
export const documentsAPI = {
  ...
  createHTMLShare: async (slug: string, options?: { expiration_type?: string, pin_protected?: boolean }) => {
    try {
      const exportData = await exportAPI.createFromDocument(slug, options);
      
      // Modify the response to use the HTML viewer path
      if (exportData && exportData.export_details && exportData.export_details.share_url) {
        exportData.export_details.share_url = exportData.export_details.share_url.replace('/shared/pdf/', '/shared/html/');
      }
  ...
  getDocumentVersions: (slug: string) => 
    api.get(`documents/${slug}/versions`),
  ...
  bulkDeletePermanently: (documentIds: number[]) => 
    api.post('documents/bulk/delete-permanently', { document_ids: documentIds }),
  ...
  formatWithAI: (content: string) => 
    api.post('format-with-ai', { content }),
  ...
  // Updated to use proper export architecture
  createPDFShare: async (slug: string, options?: { expiration_type?: string, pin_protected?: boolean }) => {
    try {
      const exportData = await exportAPI.createFromDocument(slug, options);
      return { data: exportData };
    } catch (error) {
...
export const legalAPI = {
  getLegalDocument: (type: string) => 
    api.get(`legal/document/${type}/`),
  
  getAllLegalDocuments: () => 
    api.get('legal/documents/?latest=true')
};
