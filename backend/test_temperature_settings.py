...
def test_temperature_settings(api_key, model="gpt-3.5-turbo-0125", temperatures=None, max_tokens=3000):
    """
    Test different temperature settings to see which one produces the best results.
    """
...
Match the tone, style, and voice of any examples provided."""
...
- IMPORTANT: Match the sarcastic, critical tone and colorful metaphors of the example
...
DO NOT simply return the original text with HTML tags added. Create a new, expanded text with the same vibe and style as the example documents."""
    ...
    for temp in temperatures:
        ...
        try:
            # Make the API call
            print(f"Calling OpenAI API with temperature {temp}...")
            ...
            # Print the response
            print(f"\nAPI Response for temperature {temp}:")
            ...
            filename = f"api_test_result_temp_{temp}.html"
            with open(filename, "w", encoding="utf-8") as f:
                f.write(generated_content)
            
            print(f"\n=== GENERATED CONTENT (FIRST 500 CHARS) for temperature {temp} ===")
            ...
            print(f"Error calling OpenAI API with temperature {temp}: {str(e)}")
    ...
    for temp, result in results.items():
        if "error" in result:
            ...
            print(f"Temperature {temp}: {result['completion_tokens']} tokens in {result['duration']:.2f}s, saved to {result['filename']}")
    
    return results

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Test different temperature settings")
    ...
    test_temperature_settings(args.api_key, model=args.model, max_tokens=args.max_tokens)
